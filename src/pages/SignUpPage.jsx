...
import {Input,InputPassword,InputEmail} from '../components/auth.components.jsx'
       ...
       <div className={`w-full md:w-1/3 rounded-2xl h-auto p-6 md:p-8 bg-tokena-white`}>
         <div className={`bg-tokena-blue-opacity7percent px-4 w-full h-16 rounded-xl flex items-center flex-row gap-3`}>
          ...
          <form className={`grid grid-cols-1 gap-2`}>
              <Input title={`Nom`} placeholder={`Entrer votre nom`} />
              <Input title={`Prénom`} placeholder={`Entrer votre prénom`} />
              ...
              <InputPassword title={`Confirmation du mot de passe`} placeholder={`Retapez votre mot de passe`} />
              <input type="submit" value="S'inscrire" className='bg-tokena-blue text-tokena-white rounded-3xl font-[Mono-sans-semibold] text-sm px-5 py-3' />
          </form>
       </div>
   </div>
  )
